...
/*
1、由于反异或操作只能进行最多一次，所以我们可以挑选其中一段
来进行反异或操作，而剩下的段，统计 1 的个数就可以了，于是
问题就转变成如何找到需要反异或操作的那一段；
2、考虑下面这种情况，首先考虑奇数：
a b c d e
^ ^ ^ ^ ^
e d c b a
满足可以反异或的前提是：中间这个位置必须是 0；因为自己异或自己
必然是 0 然后由于异或满足交换律，也就是 b^d == d^b ，所以就
转变成了求回文串的过程中；
同样偶数也是一样的，只不过偶数不需要判断 中间位置是 0 的情况；
3、利用 manacher 计算出每个位置作为回文串中心的最大半径 p[i]
求区间 [i, i+p[i]-1] 中的 1 的个数，代表 反异或 操作之前需要
的 1；
求区间 [i-p[i]+1, i+p[i]-1] 以外 1 的个数，
代表不进行反异或操作的段中需要的 1；
两者累加即可
*/
