...
typedef struct {
    ...
    int w[maxn];         // 新增1：每个结点的零食种类
    int fw[maxn][21];    // fw[i][j]代表从根结点到i结点，有多少个种类为j的零食
...
void LCA_PreProcess(LCA *lca, int root) {
    ...
    while (lca->front < lca->rear) {
        ...
        for (int i = lca->head[u]; i != -1; i = lca->next[i]) {
            ...
            // 新增3：树上前缀和
            for(int j = 1; j <= 20; ++j) {
                lca->fw[v][j] = lca->fw[u][j] + (lca->w[v] == j ? 1 : 0);
            }
        }
    }
}
...
int main() {
    ...
    while(q--) {
        int x, y;
        scanf("%d %d", &x, &y);
        int u = LCA_Get(&lca, x, y);
        int ans = 0;
        // 新增4：枚举每一种零食
        // 在 x->u 以及 y->u 的路径和上是否出现过
        for(int j = 1; j <= 20; ++j) {
            int val = (lca.fw[x][j] - lca.fw[u][j]) 
            + (lca.fw[y][j] - lca.fw[u][j]) 
            + (lca.w[u] == j ? 1 : 0);
            ans += (val > 0);
        }
        printf("%d\n", ans);
